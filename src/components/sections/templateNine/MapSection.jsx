...
export const TemplateNineMapSection = memo(function TemplateNineMapSection({
  id = 'map',
  title,
  locationUrl,
  address,
  ...
  return (
    <section id={id} className={`py-12 ${scroll}`}>
      <div className="mx-auto w-full max-w-[1000px] px-5">
        <h2 className="mb-5 text-center text-[16px] font-extrabold sm:text-[18px]" style={{ color: T9_NAVY }}>
          {title}
        </h2>
        {src ? (
          ...
          <p className="text-center text-[10px] text-black/45">Add address or location URL to show the map.</p>
        )}
      </div>
    </section>
  )
