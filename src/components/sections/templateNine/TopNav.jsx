...
export const TemplateNineTopNav = memo(function TemplateNineTopNav({
  brand,
  nav,
  headerLogin,
  headerSignup,
  ...
  const solidBtn =
    'inline-flex h-[32px] shrink-0 items-center justify-center rounded-[10px] px-3 text-[10px] font-semibold text-white shadow-[0px_10px_24px_rgba(13,58,92,0.22)] transition hover:opacity-95 sm:px-4'

  const mobileLinkClass =
    'block border-b border-black/[0.06] px-5 py-3.5 text-left text-[13px] font-semibold text-[#1a2744] transition hover:bg-black/[0.03]'

  return (
    <header className="sticky top-0 z-50 border-b border-black/[0.06] bg-white">
      <div className="relative">
        <div className="relative z-30 mx-auto flex max-w-[1000px] items-center justify-between gap-3 px-5 py-4">
          <a href="#top" className="min-w-0 shrink-0 hover:opacity-90" onClick={() => setOpen(false)}>
            <p className="truncate text-[12px] font-extrabold tracking-tight" style={{ color: T9_NAVY }}>
              {brand}
            </p>
          </a>

          <div className="flex min-w-0 flex-1 items-center justify-end gap-2 sm:gap-3 md:gap-5">
            <nav className="hidden items-center gap-4 text-[10px] font-semibold text-black/50 md:flex" aria-label="Primary">
              {(nav ?? []).map((item) => {
                ...
                return (
                  <a
                    ...
                    href={item.href}
                    className={cn('whitespace-nowrap hover:text-[#1a2744]', active && 'font-bold')}
                    style={{ color: active ? T9_NAVY : undefined }}
                  >
                    {item.label}
                  </a>
                )
                  ...
                  <a
                    ...
                    className={cn(
                      ...
                      isHashNavActive(item.href, docHash) &&
                        'bg-[rgba(13,58,92,0.08)] font-bold text-[#1a2744]',
                    )}
                    style={
                      isHashNavActive(item.href, docHash) ? { color: T9_NAVY } : undefined
                    }
                    onClick={() => setOpen(false)}
                  >
                    {item.label}
