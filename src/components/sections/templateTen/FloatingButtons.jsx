...
import { T10_BLUE, T10_BLUE_DARK, T10_BLUE_SOFT } from './constants.js'

function digitsOnly(s) {
  return String(s ?? '').replace(/\D/g, '')
}

export const TemplateTenFloatingButtons = memo(function TemplateTenFloatingButtons({
  phone,
  whatsapp,
  shareTitle,
  shareText,
    ...
    <div className="fixed bottom-5 right-5 z-50 flex flex-col items-end gap-3 sm:bottom-6 sm:right-6">
      {shareHint ? (
        <span
          ...
          style={{ backgroundColor: T10_BLUE_SOFT, color: T10_BLUE_DARK }}
        >
          Link copied
        </span>
      ) : null}
      <div className="flex flex-col gap-3">
        <button
          ...
          style={{ borderColor: T10_BLUE, backgroundColor: '#fff', color: T10_BLUE }}
        ...
        {tel ? (
          <a href={`tel:${tel}`} className={filledFab} style={{ backgroundColor: T10_BLUE_DARK }} aria-label="Call">
            <span aria-hidden>📞</span>
          </a>
        ) : null}
      </div>
