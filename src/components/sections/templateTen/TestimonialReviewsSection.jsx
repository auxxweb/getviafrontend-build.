...
export const TemplateTenTestimonialReviewsSection = memo(function TemplateTenTestimonialReviewsSection({
  id = 'testimonials',
  title,
  carouselItems,
  reviews,
  ...
  return (
    <section id={id} className={`border-t border-black/[0.04] py-14 ${scroll}`} style={{ backgroundColor: '#EFF6FF' }}>
      <div className="mx-auto w-full max-w-[1040px] px-5">
        {items.length > 0 ? (
          <div className="grid gap-8 lg:grid-cols-[1fr_1.05fr] lg:items-center">
            <div>
              <h2 className="text-[20px] font-extrabold leading-tight sm:text-[22px]">{title}</h2>
              {items.length > 1 ? (
            ...
            {active ? (
              <div className="relative rounded-[18px] bg-white p-6 shadow-[0px_16px_40px_rgba(15,23,42,0.08)] ring-1 ring-black/[0.06]">
                <span
                  className="pointer-events-none absolute right-5 top-3 text-[64px] font-serif font-bold leading-none opacity-[0.12]"
                  style={{ color: accent }}
                  aria-hidden
                >
                  “
                ...
                <div className="flex items-center gap-4">
                  <div className="size-14 shrink-0 overflow-hidden rounded-full ring-2 ring-blue-100">
                    <PlaceholderImage src={active.image} alt="" label="Client" className="size-full" />
                  </div>
                  <div>
                    <p className="text-[13px] font-extrabold">{active.name}</p>
                    <p className="mt-0.5 text-[12px] text-amber-500">★★★★★</p>
                  </div>
                </div>
                <p className="relative z-[1] mt-4 text-[11px] font-medium leading-[18px] text-black/50">{active.quote}</p>
              </div>
            ) : null}
          </div>
        ) : (
          ...
          <ul className="grid gap-4 sm:grid-cols-2">
            {reviews.map((r) => (
              <li
                ...
                className="rounded-[16px] bg-white p-5 shadow-[0px_12px_32px_rgba(15,23,42,0.06)] ring-1 ring-black/[0.06]"
              >
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <p className="text-[13px] font-extrabold text-[#0f172a]">{r.author}</p>
                  <StarRow value={r.rating ?? 5} readOnly />
                </div>
                {r.date ? (
                  <p className="mt-1 text-[10px] font-bold uppercase tracking-wide text-black/40">{r.date}</p>
                ) : null}
                <p className="mt-3 text-[11px] font-medium leading-[17px] text-black/50">{r.text}</p>
              </li>
            ))}
          </ul>

          <div className="rounded-[16px] bg-white p-6 ring-1 ring-black/[0.06]">
            <h3 className="text-[16px] font-extrabold">{formTitle}</h3>
            <form className="mt-5 grid gap-4" onSubmit={handleSubmit} noValidate>
              ...
              <button
                type="submit"
                className="h-[44px] w-full rounded-[10px] text-[11px] font-bold text-white shadow-[0px_10px_24px_rgba(37,99,235,0.35)]"
